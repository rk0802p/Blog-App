  ...
  return (
    <main className="min-h-screen py-12 bg-gradient-to-br from-gray-100 to-white">
      <div className="max-w-2xl mx-auto px-4">
        <h1 className="text-4xl font-extrabold mb-10 text-gray-800 border-b-2 border-teal-500 pb-4 inline-block tracking-tight">Recent Blog Posts</h1>
        <div className="grid gap-8">
          {posts.map((post) => (
            <Link
              key={post.id}
              href={`/post/${post.id}`}
              className="block p-7 bg-white rounded-xl shadow-md hover:shadow-xl transform hover:-translate-y-1 transition-all duration-300 ease-in-out border border-gray-200 hover:border-teal-500"
            >
